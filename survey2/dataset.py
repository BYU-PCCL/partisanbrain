...
class Dataset(abc.ABC):
    def __init__(self, dv, in_fname=None, opening_func=None, out_fname=None, sample_seed=0, n=None):
        self._dv = dv
        ...
        if n is not None:
            self._df = self._df.sample(n=n, random_state=sample_seed)

        self._templates = self._get_templates()

        self._result_df = self._make_result_df()

        # Save results dataframe
        save_fname = f"{recommended_ds_dir}/ds.pkl"
        self._result_df.to_pickle(out_fname or save_fname)


    @abc.abstractmethod
    def _modify_raw_data(self, df):
        """
        Modifies the raw data in df.
    ...
    def _get_templates(self):
        """
        Returns a dictionary of templates for the dataset.
        """
        pass
    ...
    def _get_n_prompts(self, n):
        """
        Get n prompts from the dataset
        Returns a list of prompts
        """
        return self._result_df.sample(n=n, random_state=0)["prompt"].tolist()