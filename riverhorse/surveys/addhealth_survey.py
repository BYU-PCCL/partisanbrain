...
class AddhealthSurvey(Survey):
    ...
    def modify_data(self, df):
        ...
        dem_cols = sum(
            [col if isinstance(col, list) else [col] for col in dem_dict.values()], []
        )
        ...
        mod_df["race_ethnicity"] = sum(
            [i * mod_df[code].values for i, code in enumerate(race_cols, 1)]
        )
        ...
        rename_dict = {
            code: dem for dem, code in dem_dict.items() if not isinstance(code, list)
        }
